...
// Alterei esse medoto para usar números formatados em português do Brasil
function convertNumber(number, config) {
    // Sempre retorna o número formatado pt-BR se >= 1000, ou simples se menor, ambos com símbolo se necessário
    var value = parseInt(number);
    var symbol = config && config.currency && typeof config.currency === "string" ? config.currency : "";
    if (!isNaN(value)) {
        if (value >= 1000) {
            return value.toLocaleString("pt-BR", { maximumFractionDigits: 0, minimumFractionDigits: 0 }) + symbol;
        } else {
            return value + symbol;
        }
    }
